...
- **Direct task/steering instructions are addressable, envelope-only, and session-scoped** (`src/directives.js`). The first user turn whose carved envelope is non-empty is the task span (`TASK-1`), later such turns are steering (`STEER-<turn>`) whatever their length, each with authority `direct-task`/`direct-steering` and a lifetime open from its own turn onward. A span is session authority, never durable memory: a mistake only a direct instruction covered is still a gap sighting, cited alongside the directive negative, so cross-session recurrence still clusters. Only the authoritative envelope indexes: fenced blocks and `>` quotes are structurally excluded, and assistant/tool text never becomes a span. Spans render in the analysis prompt by turn reference, never by duplicating turn text; only ids plus metadata persist on the evidence record. The fold has no directive rows at all: a cite of a span the session issued is dropped from memory-instruction relevance, and the recurrence signal reaches synthesis as ordinary gap sightings.
