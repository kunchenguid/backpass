...
test('an edit whose find text is missing or ambiguous is refused, never guessed at', () => {
  ...
  assert.throws(
    () => applyEdit(doubled, { id: 'e1', file: 'AGENTS.md', find: '- Whenever a PR is mentioned, include its URL.', replace: 'x' }),
    /appears 2 times/,
  );
});
...
test('an edit with no verbatim quote is rejected', () => {
  const { proposal, violations } = buildProposal(
    {
      edits: [{ kind: 'remove', file: 'AGENTS.md', title: 'unsupported', find: '- Use Node 18 via nvm before running any script.\n', replace: '', evidence: [] }],
    },
    context(),
  );
...
test('an extract edit requires a skill draft, and a non-extract must not carry one', () => {
  ...
  const missing = buildProposal(
    { edits: [{ kind: 'extract', file: 'AGENTS.md', title: 'x', find: '- Use Node 18 via nvm before running any script.', replace: 'see skill', evidence: QUOTE, transcripts: 2 }] },
    context(),
  );
  assert.ok(missing.violations.some((v) => /requires a skill draft/.test(v)));

  const stray = buildProposal(
    { edits: [{ kind: 'rewrite', file: 'AGENTS.md', title: 'x', find: '- Use Node 18 via nvm before running any script.', replace: 'y', evidence: QUOTE, transcripts: 2, skill }] },
    context(),
  );
  assert.ok(stray.violations.some((v) => /only kind "extract"/.test(v)));

  const good = buildProposal(
    { edits: [{ kind: 'extract', file: 'AGENTS.md', title: 'x', find: '- Use Node 18 via nvm before running any script.', replace: '- See the release-signing skill.', evidence: QUOTE, transcripts: 2, skill }] },
    context(),
  );
...
test('projectWithDecisions tracks the budget for the subset the human accepted', () => {
  const { proposal } = buildProposal(
    {
      edits: [
        { kind: 'remove', file: 'AGENTS.md', title: 'a', find: '- Use Node 18 via nvm before running any script.\n', replace: '', evidence: QUOTE, transcripts: 3 },
        { kind: 'rewrite', file: 'AGENTS.md', title: 'b', find: '- Whenever a PR is mentioned, include its URL.', replace: '- Whenever a PR is mentioned, include its full https:// URL before any shorthand.', evidence: QUOTE, transcripts: 3 },
      ],
    },
    context(),
  );
...
test('applying decisions writes accepted edits, skips rejected ones, and remembers rejections', () => {
  ...
  const { proposal } = buildProposal(
    {
      edits: [
        { kind: 'remove', file: 'AGENTS.md', title: 'drop node pin', find: '- Use Node 18 via nvm before running any script.\n', replace: '', evidence: QUOTE, transcripts: 3 },
        { kind: 'rewrite', file: 'AGENTS.md', title: 'pr url', find: '- Whenever a PR is mentioned, include its URL.', replace: '- Always include the full PR URL.', evidence: QUOTE, transcripts: 3 },
      ],
    },
...
test('a dry run reports what it would write without touching the file', () => {
  ...
  const { proposal } = buildProposal(
    { edits: [{ kind: 'remove', file: 'AGENTS.md', title: 'drop node pin', find: '- Use Node 18 via nvm before running any script.\n', replace: '', evidence: QUOTE, transcripts: 3 }] },
    context({ repo: { name: 'demo', root: dir } }),
  );
...
test('a file within budget keeps the strict cap gate', () => {
  const { proposal } = buildProposal(
    {
      edits: [
        { kind: 'remove', file: 'AGENTS.md', title: 'x', find: '- Use Node 18 via nvm before running any script.\n', replace: '', evidence: QUOTE, transcripts: 3 },
      ],
    },
    context(),
  );
