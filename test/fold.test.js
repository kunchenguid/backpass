...
test('near-duplicate gaps from different sessions cluster into one item', () => {
  const summary = foldEvidence(
    [
      record('s1', {
        gaps: [{ proposedInstruction: 'Read docs/db.md before writing queries.', quote: 'walked migrations', recurrenceRisk: 'high' }],
      }),
      record('s2', {
        gaps: [{ proposedInstruction: 'Read docs/db.md before writing database queries.', quote: 'rebuilt the schema', recurrenceRisk: 'medium' }],
      }),
    ],
    { minGapEvidence: 2 },
  );
...
test('a gap seen in only one session is dropped - batch size greater than one', () => {
  const summary = foldEvidence(
    [record('s1', { gaps: [{ proposedInstruction: 'Always vendor the lockfile.', quote: 'q', recurrenceRisk: 'low' }] })],
    { minGapEvidence: 2 },
  );
