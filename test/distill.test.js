...
test('redaction runs on tool input and output inside the trace', () => {
  const { trace } = distill(
    [{ kind: 'tool', name: 'Bash', input: { command: 'curl -H "token: ghp_abcdefghijklmnopqrstuvwxyz0123"' }, result: 'ok' }],
    META,
  );
...
test('evidence items without a verbatim quote are discarded at parse time', () => {
  const clean = sanitizeEvidence({
    ...
    gaps: [
      { proposedInstruction: 'Read docs/db.md before writing queries.', quote: 'walked migrations for 18 turns', recurrenceRisk: 'high' },
