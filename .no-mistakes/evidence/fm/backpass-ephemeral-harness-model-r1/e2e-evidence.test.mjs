...
const { execOneShot, openSession, sessionPrompt } = await import("file:///Users/kunchen/.no-mistakes/worktrees/26c81f74111f/01M10M04FP9JSMV2P3T7JGQKES/src/acpx.js");
const { prepareHarnessInvocation } = await import("file:///Users/kunchen/.no-mistakes/worktrees/26c81f74111f/01M10M04FP9JSMV2P3T7JGQKES/src/harness-invoke.js");
...
const cli = '/Users/kunchen/.no-mistakes/worktrees/26c81f74111f/01M10M04FP9JSMV2P3T7JGQKES/bin/backpass.js';
let cliRun = 0;
...
test("the Backpass CLI applies Pi overlays without changing persistent defaults", () => {
  ...
  const bare = spawnSync(fakePi, ["--mode", "rpc", "--no-themes"], { env: process.env, encoding: "utf8" });
  console.log("EVIDENCE " + JSON.stringify({ surface: "Backpass CLI Pi analysis", exit: result.status, harnessArgv: spawned[0], persistentSettingsByteStable: settingsBytes().compare(before) === 0, laterBarePiDefaults: JSON.parse(bare.stdout) }));
});
...
test("the Backpass CLI applies Grok model and effort as process arguments", () => {
  ...
  assert.equal(settingsBytes().compare(before), 0);
  assertBareDefaults(before);
  console.log("EVIDENCE " + JSON.stringify({ surface: "Backpass CLI Grok analysis", exit: result.status, harnessArgv: spawned[0], persistentSettingsByteStable: settingsBytes().compare(before) === 0 }));
});
...
test("the Backpass CLI preserves safe process fallbacks and rejects unsafe ones", () => {
  ...
  for (const agent of ["claude", "codex"]) {
    ...
    assert.equal(settingsBytes().compare(before), 0);
    assertBareDefaults(before);
    console.log("EVIDENCE " + JSON.stringify({ surface: `Backpass CLI unsafe ${agent} fallback`, exit: result.status, actionableError: result.stderr.split("\n").filter((line) => line.includes("cannot apply invocation-scoped") || line.includes("upgrade acpx")) }));
  }
});
...
test("the Backpass CLI keeps Pi synthesis overlays invocation-scoped", () => {
  ...
  assert.equal(settingsBytes().compare(before), 0);
  assertBareDefaults(before);
  console.log("EVIDENCE " + JSON.stringify({ surface: "Backpass CLI Pi synthesis", exit: result.status, harnessLaunches: spawned, persistentSettingsByteStable: settingsBytes().compare(before) === 0 }));
});
